...
class EmbeddingNet(nn.Module):
    def __init__(self, x_dim: int, c_dim: int, hidden: int = 128, use_layernorm: bool = False):
        ...
        if use_layernorm:
            layers.append(nn.LayerNorm(c_dim))
...
def main(args):
    ...
    embedding_net = LinearEmbedding(x_dim = X.size(1), c_dim = args.cdim if args.cdim is not None else theta.size(1)) 
...
if __name__ == "__main__":
    ...
    for i in range(len(x0_list.tolist())):
        create_c2st_job_script(args.task, args.num_training, "c2st", i, args.seed, 10_000, args.cond_den, gpu_ind, embed=True, cdim=args.cdim)
