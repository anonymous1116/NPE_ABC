...
def main(args):
    ...
    output_file_path = os.path.join(f'../depot_hyun/hyun/NPE_ABC/nets_embed/{args.task}_cdim{args.cdim}/J_{int(args.num_training/1000)}K/{args.task}_{seed}_{args.cond_den}.pkl')
    ...
    output_dir = f"../depot_hyun/hyun/NPE_ABC/flow_c2st_embed_results/{args.task}_{args.cdim}/J_{int(args.num_training/1000)}K/{int(args.L/1_000_000)}M_eta{sci_str}"
...
def get_args():
    ...
    parser.add_argument('--cdim', type=int, default=None, 
                        help='dimension of context embedding (default: None, only for embedding case)')
