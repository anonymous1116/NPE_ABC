...
def run_similiarity(task, measure, x0_ind, seed, post_n_samples, num_training, cond, method):
    ...
    if method in ["FMPE", "NPSE"]:
        output_file_path = f"../depot_hyun/hyun/NPE_ABC/{method}_nets_NABC/{task}/J_{int(num_training/1000)}K/{task}_{seed}.pkl"    
    else:    
    ...
    output_dir = f"../depot_hyun/hyun/NPE_ABC/{method}_{measure}_NABC_results/{task}/J_{int(num_training/1000)}K"   
...
#python utils/get_measure_NABC.py --task "slcp_summary_transform2" --measure "c2st" --x0_ind 1 --seed 1 --post_n_samples 10000 --num_training 1000 --cond_den "nsf" --method "FMPE"