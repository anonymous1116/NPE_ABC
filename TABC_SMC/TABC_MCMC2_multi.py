...
def run_single_chain(chain_args):
    chain_id, args, x0, seed = chain_args
    torch.manual_seed(seed + chain_id)
    np.random.seed(seed + chain_id)
    
    # --- your existing per-chain logic here ---
...
def main(args):
    num_chains = os.cpu_count()  # or set manually
    
    chain_args = [(c, args, x0, args.seed) for c in range(num_chains)]
    
    with Pool(processes=num_chains) as pool:
        results = pool.map(run_single_chain, chain_args)
    
    # results is a list of (theta_list, s_list) per chain
